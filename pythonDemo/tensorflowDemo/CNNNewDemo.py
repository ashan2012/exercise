...
# @Site    :
# @File    : CNNNewDemo.py
# @descrip :  在原来的基础cnn上增加了几点：  1、对weights进行了L2de正则化  2、对数据进行了翻转，剪切等数据增强  3、池化层后面加一层LRN层，增强模型的泛化能力
...
weights1 = variable_with_weight_loss(shape=[5,5,3,64],stddev=5e2,w1=0.0)
...
weight2 = variable_with_weight_loss(shape=[5,5,64,64],stddev=5e2,w1=0.0)
...
train_op = tf.train.AdamOptimizer(1e3).minimize(loss)
top_k_op = tf.nn.in_top_k(logits,label_holder,1)  ##1表示输出top1的准确率
